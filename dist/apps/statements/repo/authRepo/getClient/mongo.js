"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var atob_1 = __importDefault(require("atob"));
var NoModel_1 = __importDefault(require("jscommons/dist/errors/NoModel"));
var ExpiredClientError_1 = __importDefault(require("../../../errors/ExpiredClientError"));
var UntrustedClientError_1 = __importDefault(require("../../../errors/UntrustedClientError"));
var parseJson_1 = __importDefault(require("../../../utils/parseJson"));
var findClientByAccessToken = function (db, accessToken) { return __awaiter(void 0, void 0, void 0, function () {
    var accessTokenDoc, clientDoc;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, db.collection('oAuthTokens').findOne({
                    accessToken: accessToken,
                })];
            case 1:
                accessTokenDoc = _a.sent();
                if (!accessTokenDoc) {
                    return [2 /*return*/, null];
                }
                return [4 /*yield*/, db.collection('client').findOne({
                        _id: accessTokenDoc.clientId,
                    })];
            case 2:
                clientDoc = _a.sent();
                return [2 /*return*/, clientDoc];
        }
    });
}); };
var findClientByBasicAuth = function (db, encodedBasicAuthToken) { return __awaiter(void 0, void 0, void 0, function () {
    var decodedAuthToken, splitAuthToken, key, secret, clientDoc;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                decodedAuthToken = atob_1.default(encodedBasicAuthToken);
                splitAuthToken = decodedAuthToken.split(':');
                key = splitAuthToken[0], secret = splitAuthToken[1];
                return [4 /*yield*/, db.collection('client').findOne({
                        'api.basic_key': key,
                        'api.basic_secret': secret,
                    })];
            case 1:
                clientDoc = _a.sent();
                return [2 /*return*/, clientDoc];
        }
    });
}); };
var findClientWithAuth = function (db, authToken) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, authType, authValue;
    return __generator(this, function (_b) {
        _a = authToken.split(' '), authType = _a[0], authValue = _a[1];
        switch (authType) {
            case 'Basic':
                return [2 /*return*/, findClientByBasicAuth(db, authValue)];
            case 'Bearer':
                return [2 /*return*/, findClientByAccessToken(db, authValue)];
            default:
                throw new NoModel_1.default('Client');
        }
        return [2 /*return*/];
    });
}); };
exports.default = (function (config) {
    return function (_a) {
        var authToken = _a.authToken;
        return __awaiter(void 0, void 0, void 0, function () {
            var db, clientDoc, _b, orgDoc, lrsDoc, client;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0: return [4 /*yield*/, config.db()];
                    case 1:
                        db = _c.sent();
                        return [4 /*yield*/, findClientWithAuth(db, authToken)];
                    case 2:
                        clientDoc = _c.sent();
                        if (clientDoc === null || clientDoc === undefined) {
                            throw new NoModel_1.default('Client');
                        }
                        if (clientDoc.isTrusted === false) {
                            throw new UntrustedClientError_1.default();
                        }
                        return [4 /*yield*/, Promise.all([
                                db.collection('organisations').findOne({
                                    _id: clientDoc.organisation,
                                }),
                                db.collection('lrs').findOne({
                                    _id: clientDoc.lrs_id,
                                }),
                            ])];
                    case 3:
                        _b = _c.sent(), orgDoc = _b[0], lrsDoc = _b[1];
                        if (orgDoc === null || orgDoc === undefined || lrsDoc === null || lrsDoc === undefined) {
                            throw new NoModel_1.default('Client');
                        }
                        if (orgDoc.expiration !== null && orgDoc.expiration < new Date()) {
                            throw new ExpiredClientError_1.default();
                        }
                        client = {
                            _id: clientDoc._id.toString(),
                            title: clientDoc.title,
                            authority: parseJson_1.default(clientDoc.authority, ['client', 'authority']),
                            isTrusted: clientDoc.isTrusted,
                            lrs_id: clientDoc.lrs_id.toString(),
                            organisation: clientDoc.organisation.toString(),
                            scopes: clientDoc.scopes,
                        };
                        return [2 /*return*/, { client: client }];
                }
            });
        });
    };
});
//# sourceMappingURL=mongo.js.map