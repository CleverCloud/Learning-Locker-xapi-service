...
exports.default = (function (optsOverrides, content, contentType, sendVersion) {
    ...
    var req = supertest_1.default.put(constants_1.route).set('Content-Type', contentType);
    if (sendVersion) {
        req.set('X-Experience-API-Version', constants_1.xapiHeaderVersion);
    }
    return req
        .query(__assign({ activityId: activityId, agent: agent, registration: registration, stateId: stateId }, optsOverrides))
        .send(content);
});
